...
namespace ShaiyaNakedMap
{
	...
	void StartNakedMap()
	{
		g_dwNakedMap = GetPrivateProfileInt(L"Naked", L"map", 0, g_szFilePath);
		for (DWORD i = 0; i < 24; ++i)
		{
			WCHAR szKey[MAX_PATH] = { 0 };
			WCHAR   szReturn[MAX_PATH] = { 0 };
			swprintf_s(szKey, MAX_PATH, L"%d", i);
			if (GetPrivateProfileString(L"Naked", szKey, L"", szReturn, MAX_PATH, g_szFilePath) <= 0)
				continue;

			LOGD << "StartNakedMap" << szReturn;
			swscanf(szReturn, L"%d,%d", &g_ayLegalItem[i].dwItemid,&g_ayLegalItem[i].bAlow);
			LOGD << "item:" << g_ayLegalItem[i].dwItemid << "isallow:" << g_ayLegalItem[i].bAlow;
		}


		CMyInlineHook Hookobj_6;
		g_pNakeMapReturnAddr = (PVOID)Hookobj_6.Hook((PVOID)g_hookAddr.nakedMap1, Naked_NakedMap, 5);
		g_fnNakeMap = NakedMap;


		CMyInlineHook HookObj_7;
		g_pSlotLimitReturnAddr = (PVOID)HookObj_7.Hook((PVOID)0x00468377, Naked_SlotLimit, 8);
