...
{
	std::wstring szName;          //字段名
	vector<std::wstring> vecValue;//字段值
...
public:
	...
	BOOL    ExeSqlByCon(std::wstring szSql);                                         //执行sql语句,不返回记录集
	std::wstring ExeSqlByCommand(std::wstring szSql, std::wstring szFieldName);                //执语语句,获取第二个参数的返回值,一般是返回一条记录的值
	BOOL    ExeSqlByCommand(std::wstring szSql);                                     //执语句语,解析记录集到成员函数
	BOOL    ExpainRecord(_RecordsetPtr &pRecord);                               //解析记录集
	std::wstring GetValueByField(std::wstring szField);	                                //获取记录集里的内容
	...
	//7.数据库
	std::string ExecProc(std::string strProduteName, std::string tstr)
	{
		_CommandPtr cmd;
